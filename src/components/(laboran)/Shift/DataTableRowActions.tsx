...
interface DataTableRowActionsProps<TData extends Shift> { 
    table: Table<TData>
    column: Column<TData>
    row: Row<TData> 
} 

const DataTableRowActions = <TData extends Shift,>({ table, column, row }: DataTableRowActionsProps<TData>) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        notifyError('Data Shift berhasil dihapus');
        mutate("http://localhost:5000/shifts");
  ...
  return (
    <>
      ...
      <AlertDialogDelete
        ...
        mutate={() => mutate("http://localhost:5000/shifts")}
      />
      <DialogFormUpdate 
