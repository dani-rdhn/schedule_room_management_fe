...
export function DialogFormUpdate({ prioritasData, isOpen, setIsOpen, mutate }: DialogFormUpdateProps) {
  // const [isEditSheetOpen, setIsEditSheetOpen] = useState(false);
  // const [isOpen, setIsOpen] = useState(false); 
  const { data: session } = useSession(); // Use destructuring
  const [nama_modul, setNama_modul] = useState(prioritasData?.nama_modul || "");
  const [user_qty, setUser_qty] = useState(prioritasData?.user_qty || "");
  const [keterangan_modul, setKeterangan_modul] = useState(prioritasData?.keterangan_modul || "");
  const [status, setStatus] = useState(prioritasData?.status || "offline");
  const [select_pc, setSelect_pc] = useState(prioritasData?.select_pc || "Pilih kebutuhan...");
  const [priority_ruangan, setPriority_ruangan] = useState(prioritasData?.priority_ruangan || "");
  ...
  const handleEditPrioritas = async () => {
    console.log('handleEditPrioritas called');
    try {
      const res = await fetch(`http://localhost:5000/kebutuhan-modul/${prioritasData.id}`, { // Use roomData.id from the prop
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${session?.user.jwt}`,
        },
        body: JSON.stringify({ nama_modul, user_qty, keterangan_modul, status, select_pc, priority_ruangan}),
      });
  ...
  useEffect(() => {
    const fetchRuanganOptions = async () => { 
      const res = await fetch('http://localhost:5000/rooms-options'); // Tambahkan await
      const data = await res.json(); // Tambahkan await
  ...
  return (
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Menyesuaikan Kebutuhan Modul Praktikum</DialogTitle>
          <DialogDescription>
           <div> Menyesuaikan kebutuhan yang akan dibutuhkan oleh setiap modul praktikum </div>
          </DialogDescription>
        </DialogHeader>
        <form
          onSubmit={(e) => {
        ...
        <div className="">
          ...
          <div className="items-center my-3">
            <label htmlFor="nama_modul" className="font-medium">Prioritas Ruangan</label>
            <select 
              id="nama_modul" 
              value={priority_ruangan} 
              onChange={(e) => setPriority_ruangan(e.target.value)}
