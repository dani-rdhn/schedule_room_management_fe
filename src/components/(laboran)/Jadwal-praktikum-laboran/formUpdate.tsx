...
export function DialogFormUpdate({ scheduleData, isOpen, setIsOpen, mutate }: DialogFormUpdateProps) {
  ...
  const [room_id, setRoomId] = useState<string | null>(scheduleData?.room_id || "");
  // const [room_id, setRoomId] = useState<string | null>(scheduleData?.room_id?.name || null);
  ...
  const handleEditJadwal = async () => {
    try {
      ...
      if (res.ok) {
        setIsOpen(false);
        notifyWarning('Data Jadwal Praktikum berhasil diubah.');
            ...
            >
              ...
              {modulOptions.map(option => (
                <option key={option.id} value={option.id}>
                  {option.nama_modul}; {option.keterangan_modul}
                </option>
              ))}
            </select>
          </div>
