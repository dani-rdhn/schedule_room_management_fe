...
const DataTableRowActions = <TData extends DataWithId & Prioritas,>({ table, column, row }: DataTableRowActionsProps<TData>) => {
  ...
  const onEdit = () => {
    ...
    // alert(`localhost says edit pressed with id = ${(row.original as any).id}`);
    setIsDialogOpen(true);
  };

  const onDelete = async () => {
    setIsDeleteDialogOpen(true); // Buka dialog konfirmasi hapus
    // alert(`localhost says delete pressed with id = ${(row.original as any).id}`);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        notifyError('Data Kebutuhan Praktikum berhasil dihapus');
        mutate("http://localhost:5000/kebutuhan-modul-user");
      } else {
  ...
  return (
    <>
      ...
      <AlertDialogDelete
        ...
        mutate={() => mutate("http://localhost:5000/kebutuhan-modul-user")}
      />
      <DialogFormUpdate 
