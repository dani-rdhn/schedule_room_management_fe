...
//   const { data: session, status } = useSession();
  ...
  const handlePostKebutuhan = async () => {
    if (!nama_modul || !user_qty || !keterangan_modul || !select_pc || !status) {
    ...
    try {
      ...
      if (res.ok) {
        setIsOpen(false);
        notifySuccess('Data Kebutuhan Praktikum berhasil disimpan.');
        mutate();
      } else {
  ...
  <>
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <button className="bg-blue-500 w-[180px] py-3 hover:bg-blue-700 text-white font-medium px-4 rounded-md">
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <form onSubmit={(e) => { e.preventDefault(); handlePostKebutuhan(); }}>
          <div className="">
            ...
            <div className="mb-3">
              <label htmlFor="pc" className="font-medium">
                Kebutuhan PC
              </label>
              <select
                className="w-full border border-gray-300 h-[48px] rounded-lg shadow-sm py-2 px-3 focus:outline-none focus:border-sky-500 focus:ring-sky-500"
                id="pc"
                value={select_pc}
                onChange={(e) => setSelect_pc(e.target.value)}
              >
          ...
          <DialogFooter className="mt-5">
            <button className="bg-blue-500 hover:bg-blue-700 text-white md:h-[48px] h-[64px] md:text-md text-lg font-medium py-2 px-4 rounded-md">
              Submit
            </button>
          </DialogFooter>
        </form>
